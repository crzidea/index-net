...
history.normalize = (start, end) => {
  var dates = history.store.dates.sort((a, b) => a - b)
  ...
  var past = []
  return dates.slice(start, end).reduce(
    (queue, date, i) => {
      var index = i + start
      return queue.then(() => {
        return Promise.all([
...
history.addValuesFromDay = (day, index, ticker, ref) => {
  ...
  .then((rows) => {
    ...
    for (var i = 0, l = fields.length; i < l; i++) {
      ...
      if (!value) {
        log(`can not found \`${field}\` for ${ticker}, index: ${index}`)
      }
